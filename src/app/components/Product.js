...
export default function Product({product}) {
  return (
    <>
        <Link href={`/product/${product.id}`} className={`max-w-[300px] p-1.5 border border-gray-50 hover:border-gray-200 hover:shadow-xl bg-gray-100 rounded mx-auto`}>
            {
                product?.url ? (
                    <Image width={300} height={300} className="rounded cursor-pointer " alt="product-img" src={product.url + '/280'}/>
                )
                : null
            }
